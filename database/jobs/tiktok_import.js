...
{
    const choose_streamer_query_string = "SELECT id, unique_id, first_stream, last_stream FROM streamers ORDER BY last_processed ASC LIMIT 1";
    const { id, unique_id, first_stream, last_stream } = await Database.execute(choose_streamer_query_string, [ ], { one_response: true });
